...
def relu_backward(grad_y, x):
    ...
    for i in range(x.shape[0]):
        for j in range(x.shape[1]):
            grad_x[i][j] = 0 if x[i][j] < 0 else grad_y[i][j]
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return grad_x
...
def train(model, forward, backward, update, eta, num_epochs, batch_size):
    ...
    train_err = [(i[0], 1 - i[1]) for i in train_acc_list]
    valid_err = [(i[0], 1 - i[1]) for i in valid_acc_list]
    display_plot(train_err, valid_err, "Error", number=2)
...
if __name__ == "__main__":
    main()
    #a, b, test_input, c, d, test_target = load_data("toronto_face.npz")
    #mod = load("nn_model.npz")
    #inter_var = nn_forward(mod, test_input)
    #pred = softmax(inter_var["y"])
    #ts = [max(i) for i in pred]
    #ts.sort()
    #tres = ts[4]
    #uncertain = []
    #pred_uncertain = []
    #for i in range(pred.shape[0]):
            ...
            #a = a.reshape(48, 48)
            #uncertain.append(a)
            #pred_uncertain.append(np.argmax(pred[i]) + 1)
    #im1 = Image.fromarray((uncertain[0] * 255).astype(np.uint8), mode="L")
    #im1.save("im1.jpeg")
    #im2 = Image.fromarray((uncertain[1] * 255).astype(np.uint8), mode="L")
    #im2.save("im2.jpeg")
    #im3 = Image.fromarray((uncertain[2] * 255).astype(np.uint8), mode="L")
    #im3.save("im3.jpeg")
    #im4 = Image.fromarray((uncertain[3] * 255).astype(np.uint8), mode="L")
    #im4.save("im4.jpeg")
    #im5 = Image.fromarray((uncertain[4] * 255).astype(np.uint8), mode="L")
    #im5.save("im5.jpeg")
    #print(pred_uncertain)
